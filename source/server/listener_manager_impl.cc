...
ListenerManagerImpl::ListenerManagerImpl(Instance& server,
  ...
  for (uint32_t i = 0; i < server.options().concurrency(); i++) {
    // workers 是 ListenerManagerImpl 的一个属性.
    // 定义: std::vector<WorkerPtr> workers_
    // 所以 workers_ 就是一个 WorkerPtr 的数组, 作为 ListenerManagerImpl 的属性。
    // emplace_back 是容器自带的一个方法
    workers_.emplace_back(
        // 给workers List增加新的若干worker，每个woker由createWorker去实现，createWorker在worker_impl.cc中实现
...
void ListenerManagerImpl::startWorkers(GuardDog& guard_dog, std::function<void()> callback) {
  ...
  // 这些 active_listeners 将会被绑定到每个 worker 线程中。
