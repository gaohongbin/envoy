...
// 初始化 InstanceImpl
// InstanceImpl 作为 envoy 的主要业务类, 作为 MainCommonBase 中的 server_ 初始化包含了大量的信息。
// 包括 worker, admin API, CDS, LDS 等的初始化。
void InstanceImpl::initialize(Network::Address::InstanceConstSharedPtr local_address,
  ...
  // envoy 的 admin API, 即 envoy 的管理界面
  ...
  // Worker 初始化。
  // ListenerManagerImpl 中包含一个 workers_ 属性，其是 WorkerPtr数组，所以这里也是初始化 Worker。
  listener_manager_ =
  ...
  // CDS 初始化
  // cluster_manager_factory_ 声明如下所示:
