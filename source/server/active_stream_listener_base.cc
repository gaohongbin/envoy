...
// Listener 通过 Socket 建立 connection 的过程.
// 其实到了这里, 我们都在处理传输层的东西, 不管是 Dubbo 还是 Http 请求, 这里应该都是通用的, 
// 不同的只是到了 应用层以后, 该用哪种应用层协议进行解析的问题。
void ActiveStreamListenerBase::newConnection(Network::ConnectionSocketPtr&& socket,
                                             std::unique_ptr<StreamInfo::StreamInfo> stream_info) {
  // Find matching filter chain.
  // 根据 socket, 从 filter chain manager 中取出相应的 filter chain.
  const auto filter_chain = config_->filterChainManager().findFilterChain(*socket);
  // 如果 filter_chain 为空, 就是说即使 connection 建立以后，有 request 进来, 也不知道怎么处理, 因为至少要有一个 network filter 来处理request, 将请求转发到指定的 upstream cluster。
  ...
  // 下面才真正创建了 ServerConnection, 我滴个妈呀, 前面处理了那么多,这一步才建立 ServerConnection
  // ServerConnection 在创建的时候, 就会设立对应的 FileEvent, 通过 libevent 监听对应时间, 当然对应 connection 就是 READ 和 WRITE 事件。
  ...
  // network filter chain 中包含了很多 network filters
  // 其中有一个 比较特殊的 network filter 叫做 httpConnectionManager, 它不是一个单独的 filter, 它里面包含了很多个 http filter。
  ...
  // ActionConnection 其实就是对上面已经创建的 filter chain 和 ServerConnection 的一个简单封装而已。
