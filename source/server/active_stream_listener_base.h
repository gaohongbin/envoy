...
// ActiveStreamListenerBase 作为 stream listener(流监听器) 的基类, 它拥有 socket 对应的监听器过滤器链,
// socket 通过所有的 监听器过滤器后, 将创建 connection, 后续的 listener 必须重写 newActionConnection 才能获取到新创建的 connection。
