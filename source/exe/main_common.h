...
public:
  ...
  // server_ 为 unique_ptr 的智能指针, 智能指针的 get() 其实就是返回该指针。
...
public:
  ...
  // MainCommon 的 run() 方法本质为 MainCommonBase 的 run()
...
private:
  // 这里这个 main_thread_ 和 main() 方法里面的 main_thread 区别在哪里, 为什么要处理两次。
  ...
  // options_ 主要保存了 main 方法启动时的一些参数信息
