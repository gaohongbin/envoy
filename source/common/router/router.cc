...
// HttpConnectionManager 中最为重要的一个 filter, Route filter
// Route filter 一般在 filter chain 的最后一个, 决定了 http 请求的路由。
Http::FilterHeadersStatus Filter::decodeHeaders(Http::RequestHeaderMap& headers, bool end_stream) {
  ...
  // 获取 直接相应, 这里的直接相应对应 virtual service 中的配置, 就是匹配以后, 直接在 envoy 层返回结果, 不必进入业务层。
  ...
  if (debug_config && debug_config->append_cluster_) {
    // 在 debug 模式下, 修改 response 的 header, 将上游 cluster name 加入到 response 的 header 中。
  ...
  // 检查 strict_check_headers, 这个是 virtual service 里面可以配置的
  ...
  // 在流量过载的情况下, 直接进行 localReply, 返回错误, 减少上游压力。
  ...
  // GenericConnPool 是一个用于包装 HTTP 或者 TCP 的抽象连接池 API。
  // TODO  这一段里面的代码可以后面细看一下, 设计到 负载均衡策略 的实现。
  ...
  // 针对 debug 模式, 这里会将主机名和ip 附加到 response header 中。
  // 思考:  针对 response 的处理在 route 就可以处理, 因为 request 所产生的 response 必定是在同一个 worker 里面, 并且经历同一个 Conn
  // 但 request 产生的其他 request 则不一定, 可能在别的 Conn 里面。所以需要一个全局的处理。
  ...
  // 如果配置了不向上游转发请求, 返回一个空的 localReply.
  ...
  // 这下面应该是一些超时处理, 只知道大概是超时处理的一些参数,具体不知道。
  ...
  */
  ...
  // UpstreamRequest即进行路由的核心类, 初始化一个 UpstreamRequest
