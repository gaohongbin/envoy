...
// conn_manager_impl.cc 的 dispatch 方法的实现。
...
Envoy::StatusOr<size_t> ConnectionImpl::dispatchSlice(const char* slice, size_t len) {
  ...
  // dispatchSlice 的编解码的本质是调用 parser_ 的 execute 方法。
...
StatusOr<ParserStatus> ConnectionImpl::onHeadersComplete() {
  ...
  const Http::HeaderValues& header_values = Http::Headers::get();
  // 跟 http 本身相关的一些特性, 可以搜索 http 的 Upgrade 头字段进行了解。这里可以跳过往下看。
  ...
  // methodName 表示本次请求的方法时 Post, Get, Put 的哪一种。
  // 这里针对 Connect 这种方式, 做了一次校验, 不影响主流程的阅读。
  ...
  // 上面好一大堆代码, 都是一些特殊情况的处理, 真正的逻辑在这里, onHeadersCompleteBase()
  // 真的是无用的代码绕啊绕, 有用的代码就一行。
  ...
  // on this connection.
  // 返回 NoBodyData 表明处理已经完成, 后续没有 body 信息。
...
// 在这个阶段，会完成Downstream请求的Decoder和Encoder的初始化，并且提前塞好各种回调信息。
// 设置一个Codec（ServerConnection）的Decoder和Encoder。这边的Encoder即为ServerConnection自己。
// Decoder即为ActiveStream.
...
// 当 URL, header 等信息读取完成以后, 在这里才是真正进行解析。
