...
// 前面在阅读 filter_manager_impl.cc 代码的时候, 我们看到在 Read 消息来临的时候, 会调用每个 filters 的 onData 方法,
// 过一遍 filter
Network::FilterStatus ConnectionManagerImpl::onData(Buffer::Instance& data, bool) {
  if (!codec_) {
    // Http3 codec should have been instantiated by now.
    // 获取 codec, 其本质为 ServerConnection, 主要用来对数据进行编解码
    createCodec(data);
  }
  ...
  do {
    ...
    // 真正对数据进行编解码是调用 codec 的 dispatch 方法。
    ...
    // 这里只是机械的翻译一下上面的一句话, 还不太理解, 哈哈哈
    // 处理传入数据可能会释放出站数据，因此也请在此处检查是否关闭。
    checkForDeferredClose();
...
// 这里简单说一下 http_parser 和 decoder 的区别。
// parser 解析器
// decoder 解码器
// 根据编译原理里面的内容, 解析器是将某种内容解析成某种数据结构的数据。
// 解码器就是进行编解码？例如 utf-8 等等的编解码。
void ConnectionManagerImpl::ActiveStream::decodeHeaders(RequestHeaderMapPtr&& headers,
  ...
  // 验证 header 中 path 路径是否存在(分别针对 no-CONNECT  和 CONNECT 两种情况。)
  ...
  // 进行 route 匹配, 根据请求头来确定路由。
  refreshCachedRoute();

  // tracing header 相关的, 不过没看太懂
  ...
  // 设置 StreamInfoImpl 的 request_headers_
  filter_manager_.streamInfo().setRequestHeaders(*request_headers_);

  // 最终实现为 HttpConnectionManagerConfig::createFilterChain, 从代码可以看出, 生成了一堆 StreamDecoderFilter
