...
void FilterManagerImpl::onContinueReading(ActiveReadFilter* filter,
  ...
  for (; entry != upstream_filters_.end(); entry++) {
    ...
    if (!(*entry)->initialized_) {
      ...
      // 在首次建立 connection 完成时, 要执行一次 onNewConnection() 方法, 应该是做一次判断, 看是否要继续, 不过感觉这段逻辑有点别扭。
    ...
    if (read_buffer.buffer.length() > 0 || read_buffer.end_stream) {
      // 这里应该就比较清楚了, 针对每个 filter, 读取 read_buffer 中的数据, 然后执行 filter 的 onData 方法。
