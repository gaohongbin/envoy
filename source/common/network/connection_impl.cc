...
ConnectionImpl::ConnectionImpl(Event::Dispatcher& dispatcher, ConnectionSocketPtr&& socket,
  ...
  /**
  * 可以在这里看出, 在 ConnectionImpl 初始化的时候, 将 onFileEvent 方法注册为 callBack 方法, 也就是当 ConnectionImpl
  * 因为 Read 或者 Write 满足条件后, 触发 onFileEvent 方法的调用。所以进入到 onFileEvent 我们看看当有 Request 或者 Response 的时候,
  * 是怎么处理的。
  */
...
// onRead 对读取的数据进行处理。 主要处理逻辑就是进入 network filters 过一遍。
void ConnectionImpl::onRead(uint64_t read_buffer_size) {
  ...
  // 主要逻辑在这里, 每次读取数据后, 进入 network filters 过一遍。
...
/**
* onFileEvent 方法, 也是 ConnectionImpl 满足 Read、Write 后的回调方法.
*/ 
void ConnectionImpl::onFileEvent(uint32_t events) {
  ...
  // 处理 Write 事件, 这里的 Write 应该对应 send 操作。
  ...
  // 此处即为对read事件的处理，onReadReady最终会调用到onRead方法, 这里的 Read 应该对应 recv 操作。
...
void ConnectionImpl::onReadReady() {
  ...
  // method.
  // doRead 是核心读取数据的地方。是读取。从 read_buffer_ 中读取数据, 不用想, 底层肯定是系统调用喽。
  transport_wants_read_ = false;
