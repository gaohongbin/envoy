...
// TcpListenerImpl::onSocketEvent fileEvent 事件触发后真正执行的方法。
void TcpListenerImpl::onSocketEvent(short flags) {
  ...
  while (1) {
    ...
    // onSocketEvent 处理事件触发
    // onAccept 则是在条件判断完成后准备建立连接
    cb_.onAccept(
        std::make_unique<AcceptedSocketImpl>(std::move(io_handle), local_address, remote_address));
  }
}

// TcpListenerImpl 构造函数, 在构造函数中, 指定了 fileEvent,
// 本方法，我们可以看出，该 fileEvent 针对 Read 事件, 根据水平触发, 触发后调用 onSocketEvent 方法。
