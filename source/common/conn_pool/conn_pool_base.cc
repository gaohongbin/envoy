...
// tryCreateNewConnection 只是判断创建新 connection 的条件, 但实际并没有进行创建。
...
void ConnPoolImplBase::attachStreamToClient(Envoy::ConnectionPool::ActiveClient& client,
  ...
  // 如果要在 client 上增加新的 stream, 修改一些标志状态。
  // 一个 client 上可以有多个 stream, 代表多个并发。
...
ConnectionPool::Cancellable* ConnPoolImplBase::newStreamImpl(AttachContext& context) {
  ...
  // 如果有直接可用的 client, 则直接从 ready_clients_ 中获取, 同时将各种回调重新绑定。
  ...
  // 如果 stream 的 pending 队列已经满了, 就说明有很多请求都在等着处理, 已经超过限额, 只能失败。
  ...
  // 到这里, 说明 pending 队列还没满, 那就生成一个新的 stream, 并挂到 pending 队列中。
  ...
  // 前面已经判断没有新的 connection, 则准备创建一个新的 Connection, 但 tryCreateNewConnections 只是判断创建新 conn 的条件, 并没有真正进行创建。
