...
public:
   ...
   */
   // 在进行请求转发之前, 进行一次潜在的破坏性的处理.
   // 例如对 URL 的前缀进行重写, 添加部分 header 等等.
   // 出于性能的考虑, 这个方法应该仅仅只调用一次。
...
public:
   ...
   */
   // 根据传入的 HTTP 头, 确定请求的目标路由.
   // 使用匹配的路由回调, 来判断是继续执行后面的路由匹配还是立即终止。
...
// GenericConnPool 的存在是为了通过调用 newStream 来创建 GenericUpstream 句柄，从而最终调用 onPoolReady.
 ...
 */
 // UpstreamToDownstream 是一个上游数据流和下游数据流进行交互, 或者 作为 路由器组件 的接口。
