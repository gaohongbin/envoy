...
public:
   ...
   * @return status used by the filter manager to manage further filter iteration.
   */
   // 当有新的 connection 来的时候，先执行 onAccept, 来判断是否要创建这个 connection, 如果返回 false 则拒绝,
   // 也就是说直接在 envoy proxy 这里就拒绝了。
   // 所以遍历 Listener Filter Chain 其实就是遍历其中 filters, 分别调用 onAccept 方法, 只要有任何一个方法返回 false,
   // 则表示拒绝该 connection, 一般鉴权等在 Listener Filter Chain 中。
 ...
 */
/**
*  FilterChain 当然从名字上就很直接了, 就是过滤器链。
*/ 
class FilterChain {
public:
 ...
 */
/**
*  主要是用来管理 Filter chain, 其实本质是一个 map, 通过 socket 映射到 filter chain
*/
 ...
 */
/**
*  FilterChainFactory 主要用来生成 network filter chain
*/
